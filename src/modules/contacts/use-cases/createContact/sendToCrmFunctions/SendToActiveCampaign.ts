...
async function verifyAndUpdateTags(
  ...
  try {
    ...
    if (!answeredQuestion[0]) {
      ...
      await axiosInstance.delete(`/contactTags/${answeredQuestion[0].id}`);
      await addTag(answerId, userId);
    }
  } catch (err) {
...
async function sendToActiveCampaign(
  ...
  try {
    const { contactFromActive, id } = await postContactData(contactToActive);

    const allQuizTagsCrmIds = allQuizTags.map(tagsQ => tagsQ.map(t => t.crmId));
